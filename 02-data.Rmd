...
We have used 6 different datsets in this project. Since we wanted to see how different things correlate with each other (e.g. vaccination status and education level), therefore,  we chose multiple dataset to get multiple factors into consideration before concluding a result. All datasets have been downloaded from original data source.
...
**Overall Description:** This dataset has been downloaded from Center of Disease Control (CDC) website. It contains details of the vaccination for each state over a period of time. It also contains additional details on number of different kind of vaccines administered, distributed both state wise and age wise. In total this dataset has 69 columns with 21528 observations.
...
**Overall Description:** This dataset has been downloaded from Bureau of Economic Analysis (US Department of Commerce) and contains the historical Employment statistics of the US from 1998 to 2020. This will help us in correlating the overall employment trend of each state with various other dataset. It contains a total of 31 columns with 7084 observations.
...
**Column Description:** The columns `X1998 to X2020`, `GeoFIPS`, `LineCode` contain numerical values. While columns `GeoName`, `TableName`, `IndustryClassification`, `Descripition` contain categorical values.

## Education Dataset

**Link:** [Education (Department of Education) Dataset](https://eddataexpress.ed.gov/download)

**Overall Description:** Downloaded from US Department of Education website, this dataset contains the different type of students and their count in number for each state in which they received their education. It also contains the type of program they were enrolled in and along with different sub-categories of each program type. It contains 12 columns with 513721 observations.
...
**Column Description:** The only numerical column in `value` column, rest all other are categorical in nature.
...
**Link:** [History of Local Elected State Party Dataset](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/PEJ5QU)

**Overall Description:** This dataset is downloaded from Harvard Dataverse website. It contains historical state election results and details of prty which won and the candidate who got elected along with the total number of votes casted in that election. This dataset contains a total of 19 columns and 3629 observations.

**Name of Columns:** year, state, state_po, state_fips, state_cen, state_ic, office, district, stage, special, candidate, party_detailed, writein, mode, candidatevotes, totalvotes, unofficial, version, party_simplified.

**Column Description:** `year`, `candidatevote` and `totalvotes` are numerical in nature while all other columns contains categorical values.